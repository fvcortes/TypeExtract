...
There has been some notable works about Type System with Lua that we must cite. Typed Lua \cite{murbach_typed_2014} has already defined an optional type system for the language. More than enriching documentation, this extension ensures static type safety while preserving Lua idioms. Typed Lua encodes the main data structure mechanism from Lua into arrays, records, tuples and maps. It uses a bracket syntax to denote table types:
...
As opposed to Typed Lua, Pallene is designed for efficiency. It performs runtime checks to ensure type safety with a tweak flexibility. But similarly, Pallene uses type annotations. As shown in Code ~\ref{sum}, the function \textit{sum} receives an array of float and returns a single float. Pallene has a built in interoperability with Lua by sharing its runtime and data-structures. These features allow converting Lua code to Pallene code more easly. 
